...
from keras.layers import Layer, Dense, Activation, Flatten, Conv2D, MaxPooling2D
...
class CNN(object):
    ...
    def add_input_layer(self, shape=(2,),name="" ):
        ...
        if type(shape) is not tuple:
            shape = (shape,)
        self.model.add(Layer(input_shape=shape, name=name, trainable=False))
    ...
    def append_dense_layer(self, num_nodes,activation="relu",name="",trainable=True):
        """
         ...
         :return: None
         """
        self.model.add(Dense(units=num_nodes, activation=activation,
    ...
    def append_conv2d_layer(self, num_of_filters, kernel_size=3, padding='same', strides=1,
        ...
        """
         ...
         :return: Layer object
         """
        self.model.add(Conv2D(filters = num_of_filters, kernel_size = kernel_size, padding = padding,
                               strides = strides, activation = activation, name = name, trainable = trainable))
    ...
    def append_maxpooling2d_layer(self, pool_size=2, padding="same", strides=2,name=""):
        """
         ...
         :return: Layer object
         """
        self.model.add(MaxPooling2D(pool_size = pool_size, padding = padding, 
    ...
    def set_training_flag(self,layer_numbers=[],layer_names="",trainable_flag=True):
        ...
        :return: None
        """
        if layer_numbers and layer_names is list and (len(layer_numbers) and len(layer_names)) > 1:
            self.model.get_layer(index = layer_numbers[i]).trainable = trainable_flag
            self.model.get_layer(name = layer_names[i]).trainable = trainable_flag
            return
    ...
    def get_weights_without_biases(self,layer_number=None,layer_name=""):
        ...
        This means that the first layer with activation function is layer zero
         ...
         """
        try: 
            if(layer_number!= None or layer_number == 0):
    ...
    def set_weights_without_biases(self,weights,layer_number=None,layer_name=""):
        ...
        weights_matrix = layer.get_weights()
        layer.set_weights([weights, weights_matrix[1]])
    ...
    def set_biases(self,biases,layer_number=None,layer_name=""):
        ...
        weights_matrix = layer.get_weights()
        layer.set_weights([weights_matrix[0], biases])
    ...
    def load_a_model(self,model_name="",model_file_name=""):
        ...
        if(model_name):
            if(model_name in ["VGG16","VGG19"]):
                if(model_name == "VGG16"):
                    loaded_model = keras.applications.VGG16()
                    for layer in loaded_model.layers:
                        self.model.add(layer)
                    return self.model
                else:
                    loaded_model = keras.applications.VGG19()
                    for layer in loaded_model.layers:
                        self.model.add(layer)
                    return self.model
    ...
    def set_metric(self,metric):
        ...
        """
        if metric == "mse":
            self.metrics = keras.metrics.MeanSquaredError()
        if metric == "accuracy":
    ...
    def set_optimizer(self,optimizer="SGD",learning_rate=0.01,momentum=0.0):
        ...
        """
        if optimizer == "SGD":
            self.optimizer = keras.optimizer.SGD(learning_rate = learning_rate,
                                                  momentum = momentum)
        if optimizer == "RMSprop":
            self.optimizer = keras.optimizer.RMSprop(learning_rate = learning_rate,
                                                      momentum = momentum)
    ...
    def train(self, X_train, y_train, batch_size, num_epochs):
        ...
        self.model.summary()
        model_fit = self.model.fit(x = X_train, y = y_train, batch_size = batch_size, epochs = num_epochs)
...
if __name__ == "__main__":
    ...
    biases = my_cnn.get_biases(layer_number=6)
